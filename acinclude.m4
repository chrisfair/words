...
## Contributed to adaproject by Samuel Tardieu <sam@inf.enst.fr>
## With modifications by later developers.
dnl Note that there are assumptions here only true of the GNAT ada
dnl compiler (And by extension GCC's 3.1 and later which incorporate it)

dnl Usage: AM_PROG_ADA
dnl Look for an Ada compiler (ADA environment variable, then gcc, then $CC)
dnl (Note that this is a gnat compiler, this has assumptions that might not
dnl work with a general ada compiler.)

AC_DEFUN([AM_PROG_ADA],
[AC_BEFORE([AM_PROG_ADA], [AM_TRY_ADA])
 AC_REQUIRE([AC_PROG_CC])
 AC_SUBST([SUFFIXES], [".adb .ads .ali"])
 AC_CHECK_PROGS([ADA], [gcc gnatgcc adagcc ada])
 if test -z "$ADA"; then
...
AC_CHECK_PROG([GNATLS], [gnatls], [gnatls], [FAIL])
